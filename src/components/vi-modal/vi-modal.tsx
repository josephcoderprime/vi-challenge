import {
  Component, h, Prop, Event, EventEmitter, Watch, State, getAssetPath,
} from '@stencil/core';

@Component({
  tag: 'vi-modal',
  styleUrl: 'vi-modal.css',
  shadow: true,
  assetsDirs: ['assets'],
})
        ...
        <div class="modal">
          ...
          <div class="footer">
            {this._buttons.map((button, index) => (
              <vi-button onClick={index === 0 ? this.handleAction : this.handleCancel} text={button.text} appearance={index === 0 && this.appearance} />
            ))}
          </div>
        </div>
      </div>
